"""
Targeted NaN diagnostic for tridiag_2eq custom VJP backward.

We know from bisect_nan_grads.py:
- tair_profile grad: NaN
- eair_profile grad: finite (1.6e+132)
- tleaf_leaf grad: NaN
- tg_soil grad: finite

This script instruments the tridiag_2eq custom VJP backward to check if
the adjoint solve itself produces NaN, or if the NaN comes from upstream.

Run: cd src && python ../diags/debug_tridiag_vjp.py
"""
...
# ── Same init as test_grad.py ──
sys.argv = ['debug_tridiag_vjp.py', str(NAMELIST)]
...
# Disable checkpoint so VJP backward runs eagerly (can use bool/print)
...
# ── Instrument tridiag_2eq backward ──
import multilayer_canopy.MLMathToolsMod as _math_mod
from multilayer_canopy.MLMathToolsMod import _tridiag_2eq_solve

# Save the original backward rule
_orig_bwd = _math_mod._tridiag_2eq_bwd_rule

def _instrumented_bwd_rule(residuals, g):
    """Instrumented backward: checks for NaN in inputs and outputs."""
    a1, b11, b12, c1, d1, a2, b21, b22, c2, d2, _t, _q = residuals
    g_t, g_q = g

    n = len(d1)
    print(f"\n  [VJP BWD] n={n}")
    print(f"  [VJP BWD] g_t finite: {bool(jnp.all(jnp.isfinite(jnp.stack([g_t[i] for i in range(n)]))))}")
    print(f"  [VJP BWD] g_q finite: {bool(jnp.all(jnp.isfinite(jnp.stack([g_q[i] for i in range(n)]))))}")

    # Check input cotangent values
    g_t_arr = jnp.stack([g_t[i] for i in range(n)])
    g_q_arr = jnp.stack([g_q[i] for i in range(n)])
    print(f"  [VJP BWD] g_t max_abs: {float(jnp.max(jnp.abs(g_t_arr))):.4e}")
    print(f"  [VJP BWD] g_q max_abs: {float(jnp.max(jnp.abs(g_q_arr))):.4e}")

    # Check if g_t already contains NaN
    for i in range(n):
        if not bool(jnp.isfinite(g_t[i])):
            print(f"  [VJP BWD] g_t[{i}] = {float(g_t[i])}")
    for i in range(n):
        if not bool(jnp.isfinite(g_q[i])):
            print(f"  [VJP BWD] g_q[{i}] = {float(g_q[i])}")

    # Call original backward
    result = _orig_bwd(residuals, g)

    # Check output
    grad_d1 = result[4]  # grad d1
    grad_d2 = result[9]  # grad d2
    print(f"  [VJP BWD] grad_d1 finite: {bool(jnp.all(jnp.isfinite(grad_d1)))}")
    print(f"  [VJP BWD] grad_d2 finite: {bool(jnp.all(jnp.isfinite(grad_d2)))}")
    if not bool(jnp.all(jnp.isfinite(grad_d1))):
        print(f"  [VJP BWD] grad_d1 max_abs: {float(jnp.nanmax(jnp.abs(grad_d1))):.4e}")
        for i in range(n):
            if not bool(jnp.isfinite(grad_d1[i])):
                print(f"  [VJP BWD] grad_d1[{i}] = {float(grad_d1[i])}")

    return result

# Monkey-patch the VJP backward
_math_mod._tridiag_2eq_bwd_rule = _instrumented_bwd_rule
# Re-register the VJP with the instrumented backward
_math_mod.tridiag_2eq.defvjp(_math_mod._tridiag_2eq_fwd_rule, _instrumented_bwd_rule)

# Also patch at the import site in FluxProfileSolutionMod
import multilayer_canopy.MLFluxProfileSolutionMod as _fps_mod
_fps_mod.tridiag_2eq = _math_mod.tridiag_2eq
