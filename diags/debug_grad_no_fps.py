"""
Fast gradient isolation tests that skip FluxProfileSolution (avoids slow FPS JIT compilation).
Tests gradient through CanopyTurbulence and LeafPhotosynthesis only.

Run with:
    cd /burg-archive/home/al4385/clm-ml-jax
    CLM_ML_NO_CHECKPOINT=1 python diags/debug_grad_no_fps.py
"""
import os, sys
...
sys.argv = ['debug_grad_no_fps.py', namelist_file]
...
evp_list = tuple(int(v) for v in filt.exposedvegp[:num_evp])
p = int(evp_list[0])
ncan = int(mlcanopy_inst.ncan_canopy[p])

print(f"p={p}, ncan={ncan}, num_evp={num_evp}, filter={evp_list}")

from multilayer_canopy.MLCanopyFluxesMod import GridInfo, _copy_bef_state
...
# Pre-extract o2ref as a concrete Python float (stays concrete under jax.grad)
_o2ref_py = float(mlcanopy_inst.o2ref_forcing[p])
...
# ── TEST 2: Gradient through CanopyTurbulence only ────────────────────────────
...
try:
    ...
    state2 = _build_state_up_to_turb(mlcanopy_inst)
    print(f"  loss = sum(gac): {float(jnp.sum(state2.gac_profile[p, 1:ncan+1])):.4f}")
    ...
    for fn in ['tair_profile', 'gac_profile', 'ustar_canopy', 'tref_forcing']:
        g = getattr(grads2, fn)
        if g is not None:
...
# ── TEST 3: Gradient through LeafPhotosynthesis ───────────────────────────────
...
try:
    def loss_photo(m):
        ...
        m2 = LeafPhotosynthesis(num_evp, evp_list, isun, m2, grid=grid, _o2ref_py=_o2ref_py)
        m2 = LeafPhotosynthesis(num_evp, evp_list, isha, m2, grid=grid, _o2ref_py=_o2ref_py)
        return jnp.sum(m2.gs_leaf[p, 1:ncan+1, :])

    state3a = _build_state_up_to_turb(mlcanopy_inst)
    state3a = LeafBoundaryLayer(num_evp, evp_list, isun, state3a)
    state3a = LeafBoundaryLayer(num_evp, evp_list, isha, state3a)
    state3a = LeafPhotosynthesis(num_evp, evp_list, isun, state3a, grid=grid, _o2ref_py=_o2ref_py)
    state3a = LeafPhotosynthesis(num_evp, evp_list, isha, state3a, grid=grid, _o2ref_py=_o2ref_py)
    print(f"  loss = sum(gs): {float(jnp.sum(state3a.gs_leaf[p, 1:ncan+1, :])):.4f}")
    ...
    for fn in ['tair_profile', 'gac_profile', 'gs_leaf', 'tleaf_leaf', 'tref_forcing']:
        g = getattr(grads3, fn)
        if g is not None:
            print(f"  grad({fn}): NaN={bool(jnp.any(jnp.isnan(g)))}, "
                  f"max={float(jnp.nanmax(jnp.abs(g))):.3e}")
