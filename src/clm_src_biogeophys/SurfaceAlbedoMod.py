...
def SurfaceAlbedoInitTimeConst(bounds: bounds_type) -> None:
    ...
    if mxsoil_color == 8:
        ...
        _albsat = _albsat.at[:, ivis].set(jnp.array(
            [0.12, 0.11, 0.10, 0.09, 0.08, 0.07, 0.06, 0.05], dtype=jnp.float64
        ))
        _albsat = _albsat.at[:, inir].set(jnp.array(
            [0.24, 0.22, 0.20, 0.18, 0.16, 0.14, 0.12, 0.10], dtype=jnp.float64
        ))
        _albdry = _albdry.at[:, ivis].set(jnp.array(
            [0.24, 0.22, 0.20, 0.18, 0.16, 0.14, 0.12, 0.10], dtype=jnp.float64
        ))
        _albdry = _albdry.at[:, inir].set(jnp.array(
            [0.48, 0.44, 0.40, 0.36, 0.32, 0.28, 0.24, 0.20], dtype=jnp.float64
        ))
