...
def clm_drv(
    ...
    frac_veg_nosno: Array = canopystate_inst.frac_veg_nosno_patch             # Fraction of veg not covered by snow (0 or 1)
    frac_sno_eff: Array   = waterdiagnosticbulk_inst.frac_sno_eff_col        # Effective fraction of ground covered by snow
    ...
    n_lev    = nlevsno + nlevgrnd + 1                              # Fortran: -nlevsno+1:nlevgrnd → size nlevsno+nlevgrnd, offset by 1
    ...
    waterstatebulk_inst, canopystate_inst, surfalb_inst = clmData(
        fin2, _fin_soil_adjust, time_indx,
        bounds.begp, bounds.endp, bounds.begc, bounds.endc,
    ...
    surfalb_inst = SoilAlbedo(
        bounds, filter.num_nourbanc, filter.nourbanc,
        waterstatebulk_inst, surfalb_inst,
    )
    ...
    soilstate_inst = calc_soilevap_resis(
        bounds, filter.num_nolakec, filter.nolakec,
    ...
    waterdiagnosticbulk_inst = waterdiagnosticbulk_inst._replace(
        frac_sno_eff_col=frac_sno_eff
    )
    ...
    soilstate_inst, temperature_inst, tk, cv, tk_h2osfc = SoilThermProp( 
        bounds, filter.num_nolakec, filter.nolakec,
        tk, cv, tk_h2osfc,
        temperature_inst, waterdiagnosticbulk_inst, waterstatebulk_inst, water_inst,
        soilstate_inst,
    )
    ...
    soilstate_inst = SoilWater(
        bounds, filter.num_hydrologyc, filter.hydrologyc,
    ...
    mlcanopy_inst = MLCanopyFluxes(
        bounds, filter.num_exposedvegp, exposedvegp_list,
