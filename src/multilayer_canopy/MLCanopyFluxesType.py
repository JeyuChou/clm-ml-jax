...
class mlcanopy_type(NamedTuple):
    ...
    soil_et_loss_soil: jnp.ndarray    # Fraction of ET from each soil layer (-); shape (np, nlevgrnd+1)
    ...
    tbi_profile: jnp.ndarray          # Cumulative transmittance of direct beam; shape (np, nlevmlcan+1), index 0..nlevmlcan
    swbeam_profile: jnp.ndarray       # Direct beam SW above layer (W/m2); shape (np, nlevmlcan+1, numrad+1)
    swupw_profile: jnp.ndarray        # Upward diffuse SW above layer (W/m2); shape (np, nlevmlcan+1, numrad+1)
    swdwn_profile: jnp.ndarray        # Downward diffuse SW above layer (W/m2); shape (np, nlevmlcan+1, numrad+1)
    ...
    swsrc_profile: jnp.ndarray        # Layer source: absorbed SW (W/m2); shape (np, nlevmlcan+1, numrad+1)
    ...
    swleaf_mean_profile: jnp.ndarray  # Layer mean: absorbed SW per leaf (W/m2 leaf); shape (np, nlevmlcan+1, numrad+1)
    ...
    dtleaf_leaf: jnp.ndarray          # Change in tleaf over RK step (K); shape (np, nlevmlcan+1, nleaf+1, nrk+1)
    tleaf_hist_leaf: jnp.ndarray      # Leaf temperature for history files (K)
    swleaf_leaf: jnp.ndarray          # Leaf absorbed SW (W/m2 leaf); shape (np, nlevmlcan+1, nleaf+1, numrad+1)
    ...
    dlwp_leaf: jnp.ndarray            # Change in lwp over RK step (MPa); shape (np, nlevmlcan+1, nleaf+1, nrk+1)
